...
import com.kodong.underscore.map.util.DataUpdate;
...
public class IndexQuarterlyQuotientBatchConfig {
    ...
    public Step indexQuarterlyQuotientStep(JobRepository jobRepository,
                        ...
                        ItemReader<Ixqq> indexQuarterlyQuotientReader,
    ...
    @Bean
    public FlatFileItemReader<Ixqq> indexQuarterlyQuotientReader() {
    ...
    @Bean
    public ItemProcessor<Ixqq, IndexQuarterlyQuotient> indexQuarterlyQuotientProcessor(AdministrativeDistrictRepository administrativeDistrictRepository, IndexQuarterlyQuotientRepository indexQuarterlyQuotientRepository, DataUpdate dataUpdate, DataCheck dataCheck) {
        return ixqq -> {
            ...
            Ixqq checkedIxqq = dataCheck.updateIxqq(ixqq);
            AdministrativeDistrict dong = administrativeDistrictRepository
                    .findByAdministrativeCode(checkedIxqq.getAdstrdCode())
                    .orElse(null);

            // 행정동이 null일 경우 확인용 로그
            if(dong == null){
                log.info("AdministrativeCode : {}    AdministrativeName : {}", checkedIxqq.getAdstrdCode(),checkedIxqq.getAdstrdCodeName());
            }
            ...
            Optional<IndexQuarterlyQuotient> existing = indexQuarterlyQuotientRepository.findByStandardYearQuarterCodeAndAdministrativeDistrict(
                    checkedIxqq.getStandardYearQuarterCode(), dong
            );

            // convertToStore 메서드를 호출하여 Stor 객체를 Store 엔티티로 변환
            return existing.orElseGet(()-> checkedIxqq.convertToIndxQuarterlyQuotient(dong, checkedIxqq));
        };
    }

    @Bean
    public RepositoryItemReader<IndexQuarterlyQuotient> indexQuarterlyQuotientItemReader(
            IndexQuarterlyQuotientRepository repository, GlobalData globalData) {
    ...
    public ItemProcessor<IndexQuarterlyQuotient, BusinessAttraction> indexQuarterlyQuotientItemProcessor(
        ...
        return indexQuarterlyQuotient -> {
            ...
            int score;
            List<String> thresholds = globalData.getIndexQuarterlyQuotientThresholds();
            for(ServiceIndustry serviceIndustry : globalData.getServiceIndustryList()) {

                score = calculateScore(indexQuarterlyQuotient.getTrdarChngeIx(), thresholds);
    ...
    private int calculateScore(String trdarChngeIx,List<String>  thresholds) {
        if (trdarChngeIx.equals(thresholds.get(0))) {
            return 5;
        }

        if (trdarChngeIx.equals(thresholds.get(1))) {
            return 10;
        }

        if (trdarChngeIx.equals(thresholds.get(2))) {
            return 15;
        }
